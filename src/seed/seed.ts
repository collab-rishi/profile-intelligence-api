...
async function main() {
  ...
  console.log(`🚀 Bulk seeding ${profiles.length} profiles...`);

  
  const dataToSeed = profiles.map((profile: any) => ({
    id: uuidv7(),
    name: profile.name,
  ...
  console.log(`✅ Seeding completed. ${result.count} new records added.`);
}
